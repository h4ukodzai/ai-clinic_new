...
st.set_page_config(
    page_title="AI Clinic | Pharmacies Nearby",
...
with col2:
    image = Image.open("assets/pharma.jpg")
    st.image(image.resize((600, 350)))

st.title("🏪 Pharmacies Near You")
...
st.caption(f"Searching pharmacies near ZIP {zip5_user} for {first_name or 'you'} using Google Maps.")
...
def _call_places(params):
    """One call wrapper with basic status handling."""
    r = requests.get(PLACES_NEARBY_URL, params=params, timeout=30)
    data = r.json()
    status = data.get("status")
    if status not in ("OK", "ZERO_RESULTS"):
        st.warning(f"Google Places status: {status} (query={params.get('type') or params.get('keyword')})")
    return data

@st.cache_data(ttl=600, show_spinner=False)
def places_nearby_type_or_keyword(lat: float, lon: float, radius_m: int, *, place_type: str | None = None, keyword: str | None = None, max_pages: int = 3):
    """
    Nearby search by type or keyword (radius-based). Returns up to ~60 results per query.
    """
    params = {
        "key": GOOGLE_API_KEY,
        "location": f"{lat},{lon}",
    ...
    while page < max_pages:
        if token:
            ...
            time.sleep(2)  # per Google docs
        data = _call_places(params)
...
def normalize_google_place(p: dict, lat0: float, lon0: float):
    ...
    bstatus = p.get("business_status", "OPERATIONAL")
    return {
        "place_id": pid,
        "name": name,
        "address": addr,
...
def merge_dedupe(items):
    ...
    for it in items:
        ...
        if not pid:
            ...
            pid = f"{it.get('name','')}|{it.get('address','')}"
        seen[pid] = it
...
include_brands = st.checkbox("Boost chain pharmacies (Walgreens, CVS, etc.)", value=True)

go = st.button("🔍 Search Pharmacies")
if not go:
    st.stop()
...
with st.spinner("Querying Google Places..."):
    # Core types to cover both independent and chain classifications
    TYPES = ["pharmacy", "drugstore"]

    raw = []
    for t in TYPES:
        raw.extend(places_nearby_type_or_keyword(user_lat, user_lon, radius_m, place_type=t, keyword=None, max_pages=3))

    # Optional brand keyword boosters (helps when chains are typed oddly)
    if include_brands:
        BRANDS = ["Walgreens", "CVS", "Rite Aid", "Walmart Pharmacy", "Publix Pharmacy"]
        for kw in BRANDS:
            raw.extend(places_nearby_type_or_keyword(user_lat, user_lon, radius_m, place_type=None, keyword=kw, max_pages=2))

    if not raw:
        st.info("No pharmacies found from Google Places near your ZIP. Try increasing the radius.")
        st.stop()

# Normalize & dedupe
base_items = [normalize_google_place(el, user_lat, user_lon) for el in raw]
# Keep only operational businesses
base_items = [x for x in base_items if (x.get("business_status") or "OPERATIONAL") == "OPERATIONAL"]
# Filter out where distance could not be computed (rare)
base_items = [x for x in base_items if x.get("distance_mi") is not None]
# Dedupe by place_id / name+address
base_items = merge_dedupe(base_items)
# Sort by distance then name
base_items.sort(key=lambda x: (x["distance_mi"], x["name"]))

# Fetch details for candidates we'll display
detail_take = min(len(base_items), max(how_many * 3, how_many + 15))
...
def render_cards(items, limit):
    ...
    for i, p in enumerate(items, start=1):
        with st.container(border=True):
            ...
            st.caption(" • ".join(bits) if bits else "Pharmacy")
            ...
            with colA:
                link_btn("🧭 Open in Google Maps", p["maps_url"], fill_width=True)
            with colB:
                if p.get("phone_tel"):
...
with tab_open:
    ...
    if not open_now_items:
        st.info("No pharmacies currently open found in this radius.")
    else:
...
with tab_24:
    ...
    if not open_24_items:
        st.info("No 'Open 24 hours' pharmacies found from Google data here. Try a larger radius.")
    else:
...
st.page_link("pages/3_otc_medication.py", label="OTC Medications", icon="💊")
